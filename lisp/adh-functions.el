...
(defun adh-set-font (family height)
  (if (member family (font-family-list))
      (progn
        (adh--apply-font family height (selected-frame))
        (when adh--font-hook
          (remove-hook 'after-make-frame-functions adh--font-hook))
        (setq adh--font-hook
              (lambda (frame) (adh--apply-font family height frame)))
...
(defun adh--duplicate-buffer-substring (boundaries)
...
;; Taken from https://github.com/protesilaos/dotfiles/blob/master/emacs/.emacs.d/prot-emacs.org
(defun adh-duplicate-dwim ()
  (interactive)
  (unless mark-ring
    (push-mark (point) t nil))
  (adh--duplicate-buffer-substring
   (if (region-active-p)
